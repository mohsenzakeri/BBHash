...
void bit_vector::reset_index(uint64_t index) {
    ...
    uint64_t bit = 1;
    uint64_t resetter = ~(bit<<arr_index);
    //std::cout<< std::bitset<sizeof(uint64_t)*8>(resetter) << " go \n";
    //std::cout<< std::bitset<sizeof(uint64_t)*8>(arr[arr_number]) << "\n";
...
uint64_t bit_vector::num_ones() {
    uint64_t res = 0;
    for (int i = 0; i < length; i++)
        res += this->read_bit(i) == 1 ? 1 : 0;
    return res;
}

uint64_t bit_vector::num_zeros() {
    return length - this->num_ones();
}
